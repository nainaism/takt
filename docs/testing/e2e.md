...
- Task status persistence（`e2e/specs/task-status-persistence.e2e.ts`）
  - 目的: 成功時/失敗時の `tasks.yaml` 状態遷移（完了消込・失敗記録）および `--ignore-exceed` の動作を確認。
  - `--ignore-exceed` テストケース:
    - `--ignore-exceed` 付きで `max_steps=1` の2ステップ workflow を実行 → タスクが `completed` になることを確認。
    - `--ignore-exceed` なしで同じ workflow を実行 → タスクが `exceeded` になることを確認。
