...
- Task status persistence（`e2e/specs/task-status-persistence.e2e.ts`）
  - 目的: 成功時/失敗時の `tasks.yaml` 状態遷移に加え、`max_steps` 到達時に通常実行では `exceeded`、`takt run --ignore-exceed` では `completed` になる分岐を確認。
...
- Help command（`e2e/specs/cli-help.e2e.ts`）
  - 目的: `takt --help` と `takt run --help` の表示内容、および `takt run` 専用の `--ignore-exceed` が help に出ることを確認。
